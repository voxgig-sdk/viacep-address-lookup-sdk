# viacep-address-lookup-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ViacepAddressLookup SDK as
two agent tools — `viacep-address-lookup_list` and `viacep-address-lookup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/viacep-address-lookup-mcp)
make build

# 2. Provide credentials via the environment
export VIACEP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user viacep-address-lookup \
  -- /absolute/path/to/viacep-address-lookup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./viacep-address-lookup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// viacep-address-lookup_list: first page of records
{ "entity": "cep_lookup" }
{ "entity": "cep_lookup", "query": { } }

// viacep-address-lookup_load: one record by id
{ "entity": "cep_lookup", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/viacep-address-lookup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user viacep-address-lookup \
     -- "$PWD"/dist/*/viacep-address-lookup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `viacep-address-lookup_list` and `viacep-address-lookup_load` tools now appear
   in new sessions. Ask the agent to *"list cep_lookup using viacep-address-lookup"*
   and it calls `viacep-address-lookup_list` with `{"entity":"cep_lookup"}`.
...
```sh
export VIACEP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx            # API key
export VIACEP_ADDRESS_LOOKUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `viacep-address-lookup_list` tool
...
### Call the `viacep-address-lookup_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `viacep-address-lookup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `viacep-address-lookup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VIACEP_ADDRESS_LOOKUP_APIKEY` | API key sent with every request. |
| `VIACEP_ADDRESS_LOOKUP_BASE` | Optional override of the API base URL. |
...
```sh
./viacep-address-lookup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"viacep-address-lookup_load","arguments":{"entity":"cep_lookup","query":{"id":1}}}}'
```
