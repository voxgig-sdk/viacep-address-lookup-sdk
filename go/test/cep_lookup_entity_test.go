...
func TestCepLookupEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		cepLookupRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.cep_lookup")))
...
func cep_lookupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VIACEP_ADDRESS_LOOKUP_TEST_CEP_LOOKUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VIACEP_ADDRESS_LOOKUP_TEST_CEP_LOOKUP_ENTID"])
