...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VIACEPADDRESSLOOKUP_TEST_LIVE") == "TRUE" ||
		os.Getenv("VIACEPADDRESSLOOKUP_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VIACEPADDRESSLOOKUP_TEST_EXPLAIN"); explain != "" {
