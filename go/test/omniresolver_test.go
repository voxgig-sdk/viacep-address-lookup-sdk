...
import (
	...
	omni "github.com/voxgig-sdk/viacep-address-lookup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ViacepAddressLookupError); is {
				out := map[string]any{
					"name":    "ViacepAddressLookupError",
...
func omniCtx(arg any, client *sdk.ViacepAddressLookupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ViacepAddressLookupSDK); is {
			client = live
