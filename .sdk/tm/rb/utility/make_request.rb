...
module ViacepAddressLookupUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ViacepAddressLookupResponse.new({})
    ...
    if fetch_err
      ...
      response = ViacepAddressLookupResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ViacepAddressLookupResponse.new(fetched)
    else
