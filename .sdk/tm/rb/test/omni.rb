...
module ViacepAddressLookupOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if ViacepAddressLookupOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = ViacepAddressLookupOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| ViacepAddressLookupOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || ViacepAddressLookupOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = ViacepAddressLookupSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = ViacepAddressLookupError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { ViacepAddressLookupOmni.errify(err) }
