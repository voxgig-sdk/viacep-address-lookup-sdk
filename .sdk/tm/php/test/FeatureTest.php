...
// ViacepAddressLookup SDK feature test
//
...
class FtCtrl extends ViacepAddressLookupControl
{
...
class FtClient
{
    ...
    private ViacepAddressLookupUtility $utility;

    public function __construct(ViacepAddressLookupUtility $utility, array $options)
    {
    ...
    public function get_utility(): ViacepAddressLookupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ViacepAddressLookupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ViacepAddressLookupContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ViacepAddressLookupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ViacepAddressLookupConfig::make_feature($name);
    ...
    private static function buildUrl(ViacepAddressLookupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ViacepAddressLookupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ViacepAddressLookupResponse($fetched);
        }
    ...
    private function fail(ViacepAddressLookupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ViacepAddressLookupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
