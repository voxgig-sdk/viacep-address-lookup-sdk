...
// ViacepAddressLookup SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ViacepAddressLookupTestFeature extends ViacepAddressLookupBaseFeature
{
    ...
    public function init(ViacepAddressLookupContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ViacepAddressLookupContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ViacepAddressLookupParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ViacepAddressLookupContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ViacepAddressLookupContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ViacepAddressLookupParam::call($ctx, $k);
