...
// ViacepAddressLookup SDK retry feature
...
class ViacepAddressLookupRetryFeature extends ViacepAddressLookupBaseFeature
{
    ...
    public function init(ViacepAddressLookupContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ViacepAddressLookupContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ViacepAddressLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ViacepAddressLookupContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
