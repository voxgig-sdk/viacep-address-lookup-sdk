...
boru-driven command-line client **and** interactive REPL for the ViacepAddressLookup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/viacep-address-lookup-cli)
make build

# 2. See usage (words, entities, env vars)
./viacep-address-lookup-cli --help

# 3. Provide credentials once, via the environment
export VIACEP_ADDRESS_LOOKUP_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./viacep-address-lookup-cli load 1 cep_lookup            # {id:1} shorthand
./viacep-address-lookup-cli load '{id:1}' cep_lookup       # explicit match map

# 5. Override the API base URL for a single call
VIACEP_ADDRESS_LOOKUP_BASE=https://api.example.com ./viacep-address-lookup-cli load 1 cep_lookup

# 6. No arguments -> interactive REPL
./viacep-address-lookup-cli
viacep-address-lookup> load 1 cep_lookup
viacep-address-lookup> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/viacep-address-lookup-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/viacep-address-lookup-cli load 1 cep_lookup
   ```
...
```sh
./viacep-address-lookup-cli load 1 cep_lookup          # scalar shorthand for {id:1}
./viacep-address-lookup-cli load '{id:1}' cep_lookup     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `viacep-address-lookup>`). Each line is
evaluated as its own boru expression:

```text
$ ./viacep-address-lookup-cli
viacep-address-lookup> load 1 cep_lookup
viacep-address-lookup> /help
viacep-address-lookup> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cep_lookup`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/viacep-address-lookup-cli`. |
...
(`./viacep-address-lookup-cli <expr>`) and interactively (the REPL), and expressions compose the
