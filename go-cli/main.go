// Command ViacepAddressLookup-cli is a CLI + REPL for the ViacepAddressLookup SDK,
...
const prompt = "viacep-address-lookup"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cep_lookup"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      VIACEP_ADDRESS_LOOKUP_APIKEY (api key), VIACEP_ADDRESS_LOOKUP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: VIACEP_ADDRESS_LOOKUP_APIKEY carries the API key and
	// VIACEP_ADDRESS_LOOKUP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("VIACEP_ADDRESS_LOOKUP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("VIACEP_ADDRESS_LOOKUP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewViacepAddressLookupSDK(opts)
