...
// ViacepAddressLookup SDK CepLookup entity
...
class CepLookupEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CepLookupEntity($this->_client, $opts);
    }

    /**
     * @param CepLookup|array $args CepLookup data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ViacepAddressLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CepLookup|array The current CepLookup data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CepLookup fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ViacepAddressLookupHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CepLookup fields).
     */
    ...
    /**
     * Load a single CepLookup.
     *
     * @param CepLookupLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CepLookupLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CepLookup|array The loaded CepLookup as an assoc-array at the
     *   SDK boundary; throws ViacepAddressLookupError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ViacepAddressLookupHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
