...
// ViacepAddressLookup SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ViacepAddressLookupOmni::makeStructRunner(
    ...
    {
        return ViacepAddressLookupOmni::objmodel(ViacepAddressLookupOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ViacepAddressLookupOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ViacepAddressLookupOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ViacepAddressLookupOmni::structfix($now, Struct::undef()))
                        !== json_encode(ViacepAddressLookupOmni::structfix($val, Struct::undef()))) {
