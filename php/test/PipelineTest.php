...
// ViacepAddressLookup SDK pipeline test
//
...
{
    private static function utility(): ViacepAddressLookupUtility
    {
        return new ViacepAddressLookupUtility();
    }
    ...
    private static function ctx(array $over = []): ViacepAddressLookupContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ViacepAddressLookupContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ViacepAddressLookupError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ViacepAddressLookupMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ViacepAddressLookupMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ViacepAddressLookupMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ViacepAddressLookupSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ViacepAddressLookupMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ViacepAddressLookupResponse([]),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        [, $err] = ViacepAddressLookupMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec([]),
            'response' => null,
            'result' => new ViacepAddressLookupResult([]),
        ]);
        [, $err] = ViacepAddressLookupMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec(['step' => 's']),
            'response' => new ViacepAddressLookupResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        [, $err] = ViacepAddressLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec(['step' => 's']),
            'response' => new ViacepAddressLookupResponse(self::resp(200, ['v' => 1])),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        [, $err] = ViacepAddressLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec(['step' => 's']),
            'response' => new ViacepAddressLookupResponse(self::resp(200, ['v' => 2])),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ViacepAddressLookupMakeResponse::call($ctx);
    ...
    {
        $preset = new ViacepAddressLookupResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec([]),
            'response' => new ViacepAddressLookupResponse([]),
            'result' => new ViacepAddressLookupResult([]),
        ...
        [$got, $err] = ViacepAddressLookupMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ViacepAddressLookupResult([])]);
        [, $err] = ViacepAddressLookupMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ViacepAddressLookupSpec([]), 'result' => null]);
        [, $err] = ViacepAddressLookupMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ViacepAddressLookupSpec(['step' => 's']),
            'result' => new ViacepAddressLookupResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ViacepAddressLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ViacepAddressLookupSpec(['step' => 's']),
            'result' => new ViacepAddressLookupResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ViacepAddressLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ViacepAddressLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ViacepAddressLookupResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ViacepAddressLookupSpec([]),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ViacepAddressLookupMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ViacepAddressLookupMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ViacepAddressLookupError('boom', 'boom');
        $utility->fetcher = function (ViacepAddressLookupContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ViacepAddressLookupSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ViacepAddressLookupMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ViacepAddressLookupContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ViacepAddressLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ViacepAddressLookupResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ViacepAddressLookupMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ViacepAddressLookupContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ViacepAddressLookupSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ViacepAddressLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ViacepAddressLookupResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ViacepAddressLookupSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ViacepAddressLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ViacepAddressLookupMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ViacepAddressLookupMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ViacepAddressLookupResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ViacepAddressLookupDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ViacepAddressLookupResult(['ok' => false])]);
        $this->expectException(ViacepAddressLookupError::class);
        ViacepAddressLookupDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ViacepAddressLookupResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ViacepAddressLookupMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ViacepAddressLookupMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ViacepAddressLookupBaseFeature();
        $b = new ViacepAddressLookupBaseFeature();
        ViacepAddressLookupFeatureAdd::call($ctx, $a);
        ViacepAddressLookupFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ViacepAddressLookupBaseFeature
    {
        $f = new ViacepAddressLookupBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ViacepAddressLookupFeatureAdd::call($ctx, self::named_feature('a'));
        ViacepAddressLookupFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ViacepAddressLookupFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ViacepAddressLookupFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ViacepAddressLookupFeatureAdd::call($ctx, $replace);
        ...
        ViacepAddressLookupFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ViacepAddressLookupContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ViacepAddressLookupSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ViacepAddressLookupPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ViacepAddressLookupPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ViacepAddressLookupResponse(['headers' => null]),
            'result' => new ViacepAddressLookupResult([]),
        ]);
        ViacepAddressLookupResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ViacepAddressLookupResultBody::call($ctx);
