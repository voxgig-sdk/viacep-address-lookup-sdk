...
//    only - a typed ViacepAddressLookupContext there would make every ctx
...
final class ViacepAddressLookupOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ViacepAddressLookupContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ViacepAddressLookupSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ViacepAddressLookupResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ViacepAddressLookupError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ViacepAddressLookupResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ViacepAddressLookupError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ViacepAddressLookupError(is_string($code) ? $code : '', $msg);
    }
