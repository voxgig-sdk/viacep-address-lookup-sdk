# ViacepAddressLookup PHP SDK



The PHP SDK for the ViacepAddressLookup API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->CepLookup()` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
$client = ViacepAddressLookupSDK::test($testopts, $sdkopts);
```
...
```
php/
├── viacepaddresslookup_sdk.php          -- Main SDK class
...
The main class (`viacepaddresslookup_sdk.php`) exports the SDK class
