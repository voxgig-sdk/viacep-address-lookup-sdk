...
The Lua SDK for the ViacepAddressLookup API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CepLookup(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ViacepAddressLookup(nil):load(
  { id = "test01" }, nil
)
