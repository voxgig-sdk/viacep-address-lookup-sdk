    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set VIACEP_ADDRESS_LOOKUP_TEST_CEP_LOOKUP_ENTID JSON to run live")
        client = setup["client"]
        ...
        cep_lookup_ref01_data_dt0_loaded = cep_lookup_ref01_ent.load(cep_lookup_ref01_match_dt0, None)
...
def _cep_lookup_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("VIACEP_ADDRESS_LOOKUP_TEST_CEP_LOOKUP_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("VIACEP_ADDRESS_LOOKUP_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("VIACEP_ADDRESS_LOOKUP_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("VIACEP_ADDRESS_LOOKUP_TEST_EXPLAIN") == "TRUE",
        "live": _live,
