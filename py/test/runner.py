# ViacepAddressLookup SDK test runner
...
class ViacepAddressLookupTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ViacepAddressLookupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ViacepAddressLookupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ViacepAddressLookupTestRunner.getenv("VIACEP_ADDRESS_LOOKUP_TEST_LIVE")
        override = ViacepAddressLookupTestRunner.getenv("VIACEP_ADDRESS_LOOKUP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ViacepAddressLookupTestRunner.getenv(key)
        ...
        explain = ViacepAddressLookupTestRunner.getenv("VIACEP_ADDRESS_LOOKUP_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ViacepAddressLookupTestRunner._test_control is not None:
            return ViacepAddressLookupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ViacepAddressLookupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ViacepAddressLookupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ViacepAddressLookupTestRunner.load_test_control()
...
def load_env_local():
    ViacepAddressLookupTestRunner.load_env_local()


def env_override(m):
    return ViacepAddressLookupTestRunner.env_override(m)


def entity_data(v):
    return ViacepAddressLookupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ViacepAddressLookupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ViacepAddressLookupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ViacepAddressLookupTestRunner.load_test_control()


def live_client_options():
    return ViacepAddressLookupTestRunner.live_client_options()


def live_delay_ms():
    return ViacepAddressLookupTestRunner.live_delay_ms()
