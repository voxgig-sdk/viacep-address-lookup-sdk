...
class CepLookupDirectTest < Minitest::Test
  def test_direct_load_cep_lookup
    setup = cep_lookup_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-cep_lookup", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cep_lookup_direct_setup(mockres)
