...
require_relative 'ViacepAddressLookup_types'
...
class ViacepAddressLookupSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ViacepAddressLookupHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ViacepAddressLookupHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ViacepAddressLookupFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ViacepAddressLookupUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ViacepAddressLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ViacepAddressLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ViacepAddressLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ViacepAddressLookupError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ViacepAddressLookupError => err
    ...
    if fetched.is_a?(Hash)
      status = ViacepAddressLookupHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ViacepAddressLookupError.new(
        "graphql_error", "ViacepAddressLookupSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ViacepAddressLookupSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
