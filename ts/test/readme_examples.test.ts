...
//     (\`new ViacepAddressLookupSDK(...)\` / \`ViacepAddressLookupSDK.test(...)\` become
//     \`ViacepAddressLookupSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ViacepAddressLookupSDK'
...
const TEST_SEED = {"entity":{"cep_lookup":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ViacepAddressLookupSDK(...)\` and any
// \`ViacepAddressLookupSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ViacepAddressLookupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ViacepAddressLookupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
